...
gray2int = torch.FloatTensor(256)
for n in range(256):
    gray2int[n^(n >> 1)] = (n - 128) / 256 * pi ;          # ∈[-pi/2, pi/2)
...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        ccmat = (torch.tensor([1, 2, 4, 8, 16, 32, 64, 128], device=x.device) * (x[:, class_index:] > 0.5)).sum(
            axis=1)  # (np)
        ccmat = gray2int[ccmat].to(x.device)  # (np,)
        theta_pred = ccmat[:, None]  # (np, 1)
